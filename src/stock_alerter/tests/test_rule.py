...
class PriceRuleTest(unittest.TestCase):
    ...
    def test_a_PriceRule_is_False_if_the_stock_is_not_in_the_exchange(self):
        ...
        self.assertFalse(rule.mathces(self.exchange))
...
class AndRuleTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        goog.update(datetime(2014, 2, 10), 10)
        goog.update(datetime(2014, 2, 10), 12)
        msft = Stock("MSFT")
        msft.update(datetime(2014, 2, 10), 10)
        msft.update(datetime(2014, 2, 10), 12)
        msft.update(datetime(2014, 2, 10), 14)
        redhat = Stock("RHT")
        redhat.update(datetime(2014, 2, 10), 7)
        cls.exchange = {"GOOG": goog, "MSFT": msft, "RHT": redhat}

    def test_an_AndRule_matches_if_all_component_rule_are_true(self):
